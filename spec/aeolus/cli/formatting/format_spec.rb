...
describe Aeolus::Cli::Formatting::Format do

  let(:shell) { double('shell') }
  let(:format) { Aeolus::Cli::Formatting::Format.new(shell) }
  ...
  context "with a presenter" do
    ...
    context "looking up for non-registered class" do
      ...
      it do
        expect { subject }
          .to raise_error Aeolus::Cli::Formatting::PresenterMissingError
      end
    end
    ...
    context "#presenters_for" do
      ...
      before do
        Aeolus::Cli::Formatting::PresenterSorter
          .should_receive(:new)
